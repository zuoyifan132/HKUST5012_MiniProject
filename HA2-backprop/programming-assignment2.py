...
def compute_neural_net_loss(params, X, y, reg=0.0):
    ...
    #--------second_softmax_layer--------#
    scores = h @ W2 + b2
    sigma_z = np.exp(scores) 

    # computue probability
    epsilon = 1e-12
    ...
    NLL_div_N = NLL/N
    loss = NLL_div_N + 0.5 * reg * (np.sum(W1**2) + np.sum(W2**2))
    ...
    # back propagation
    prob_copy = np.copy(prob)                   # copy the probability maxtrix 
    prob_copy[np.arange(N), y] -= 1             # error of softmax
    #dh = np.dot(prob_copy, W2.T)
    dz = np.dot(prob_copy, W2.T) * (z > 0)      # error(W1) = W2 * error(softmax) 

    # compute gradient for parameters
    dW2 = np.dot(h.T, prob_copy) / N            # input * error = h * prob_copy
    db2 = np.sum(prob_copy, axis=0) / N         # (C,)
    dW1 = np.dot(X.T, dz) / N                   # input * error = X * error(w1)
    db1 = np.sum(dz, axis=0) / N                # (H,)
